...
class UsageStatsHelper(private val context: Context) {
    ...
    fun getTodayUsageMillis(packageName: String): Long {
        ...
        // FIXED: Filter out old buckets and pick the latest active usage bucket for today
        return stats
            .filter { it.packageName == packageName && it.lastTimeUsed >= startOfToday }
    ...
    private fun getYesterdayUsageMillis(packageName: String): Long {
        ...
        return stats
            .filter { it.packageName == packageName && it.lastTimeUsed in startOfYesterday until endOfYesterday }
            .maxByOrNull { it.lastTimeUsed }
            ?.totalTimeInForeground ?: 0L
    }
    ...
    private fun getWeeklyUsageMillis(packageName: String): Long {
        ...
        return stats
            .filter { it.packageName == packageName && it.lastTimeUsed >= startOf7DaysAgo }
