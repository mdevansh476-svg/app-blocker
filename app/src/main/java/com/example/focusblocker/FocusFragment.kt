...
class FocusFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        // Tap the timer text to open the Minutes & Seconds input dialog
        tvCustomTime.setOnClickListener {
            val ctx = context ?: return@setOnClickListener
            val prefs = ctx.getSharedPreferences("FocusPrefs", Context.MODE_PRIVATE)
            val isActive = prefs.getBoolean("is_session_active", false)
            if (!isActive) {
                showMinuteSecondInputDialog()
            }
        }
    ...
    private fun showMinuteSecondInputDialog() {
        ...
        val etMinutes = EditText(context).apply {
            inputType = InputType.TYPE_CLASS_NUMBER
            hint = "Minutes (e.g. 90)"
            setText(customMinutes.toString())
        }

        val etSeconds = EditText(context).apply {
            inputType = InputType.TYPE_CLASS_NUMBER
            hint = "Seconds (0-59)"
            setText(customSeconds.toString())
        }
        ...
        AlertDialog.Builder(context)
            ...
            .setPositiveButton("Confirm") { _, _ ->
                ...
                if (mins >= 0 && secs in 0..59 && (mins > 0 || secs > 0)) {
                    customMinutes = mins
                    customSeconds = secs
                    updateTimerDisplay()
                } else {
                    Toast.makeText(context, "Enter valid minutes and seconds (0-59)", Toast.LENGTH_SHORT).show()
                }
            }
    ...
    private fun startFocusSession() {
        ...
        val totalMillis = ((customMinutes * 60L) + customSeconds) * 1000L
        val serviceIntent = Intent(ctx, FocusService::class.java).apply {
            putExtra("DURATION_MILLIS", totalMillis)
            putExtra("DURATION_MINUTES", customMinutes) // kept for history database logging compatibility
        }
